...
export function useContractions() {
  ...
  const update = useCallback((updated: ContractionEntry) => {
    replaceInList(STORAGE_KEYS.contractions, updated).then(setEntries);
    syncEntry('contractions', updated.id, updated);
  }, []);
